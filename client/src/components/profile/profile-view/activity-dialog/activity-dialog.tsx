...
> = ({
  ...
  // نوع نمودار: ستونی، دایره‌ای، خطی، تایم‌لاین
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='max-w-3xl w-full p-2 sm:p-4 lg:p-6' dir='rtl'>
        {isSkeleton ? (
          <ActivityTimelineSkeleton title={title} username={username} />
        ) : (
          <>
            ...
            {isError && (
              <div className='mt-2 mb-1 flex items-start gap-2 rounded-md border border-red-200 bg-red-50 px-2 py-1 text-xs dark:border-red-800 dark:bg-red-950/40 dark:text-red-200'>
                ...
                <div>
                  <div>در دریافت آمار فعالیت مشکلی رخ داد.</div>
                  {((error as any)?.response?.status === 404 ||
                    (error as any)?.message) && (
                    <div className='mt-1 text-xs opacity-80'>
                      {(error as any)?.response?.status === 404
                        ? 'برای این کاربر در این تاریخ داده‌ای یافت نشد.'
                        : (error as any)?.message}
                    </div>
                  )}
                </div>
                      ...
                      {chartTabs.map((tab) => {
                        ...
                        return (
                          <button
                            ...
                            className={`group flex items-center transition-all duration-300 px-2 py-1 ${
                              isActive
