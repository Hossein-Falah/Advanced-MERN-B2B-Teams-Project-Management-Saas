  ...
  if (!log) {
    return (
      <div
        ...
        dir='rtl'
      >
        {t('tasks.taskLogs.noLogSelected')}
      </div>
    )
  }
    ...
    >
      ...
      >
        <div className='space-y-1'>
          <div className='flex items-center gap-2'>
            ...
            <p className='text-sm sm:text-[15px] font-semibold'>
              {t('tasks.taskLogs.headerTitle')}
            </p>
          </div>

          <p className='text-[11px] sm:text-xs text-muted-foreground leading-5 '>
            {t('tasks.taskLogs.changesByUserAndDate', {
              userName: log.user?.name || t('tasks.taskLogs.fallbackUser'),
              date: formattedDate,
            })}
          </p>
        </div>
        ...
        <div className='flex items-center gap-2 justify-end'>
          ...
          >
            {isUndoLoading ? (
              <span>{t('tasks.taskLogs.undo.loading')}</span>
            ) : (
              <>
                <span className='hidden sm:inline'>
                  {t('tasks.taskLogs.undo.label.desktop')}
                </span>
                <span className='sm:hidden'>
                  {t('tasks.taskLogs.undo.label.mobile')}
                </span>
              </>
            )}
          </Button>
          ...
          >
            {isRedoLoading ? (
              <span>{t('tasks.taskLogs.redo.loading')}</span>
            ) : (
              <>
                <span className='hidden sm:inline'>
                  {t('tasks.taskLogs.redo.label.desktop')}
                </span>
                <span className='sm:hidden'>
                  {t('tasks.taskLogs.redo.label.mobile')}
                </span>
              </>
            )}
          </Button>
      ...
      <div className='flex-1 overflow-y-auto px-3 sm:px-4 py-3 sm:py-3.5 space-y-2.5 pr-1 custom-scrollbar'>
          ...
          >
            ...
            <div className='flex items-center justify-between gap-2 mb-2.5'>
              <p className='text-[11px] sm:text-xs font-semibold flex items-center gap-1.5'>
                ...
                <span>{getFieldLabelMap(t)[change.field]}</span>
              </p>

              <span className='text-[10px] text-muted-foreground'>
                {change.oldValue
                  ? t('tasks.taskLogs.changeType.edit')
                  : t('tasks.taskLogs.changeType.create')}
              </span>
            </div>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-3 text-[11px] sm:text-xs'>
              <div className='space-y-1.5'>
                <p className='text-[10px] text-muted-foreground flex items-center gap-1'>
                  ...
                  {t('tasks.taskLogs.previousValue.label')}
                </p>
                <div
                  className={cn(
                ...
                >
                  {renderLogFieldValue(change.field, change.oldValue, {
                    task,
                    t,
                  })}
                </div>
              ...
              <div className='space-y-1.5'>
                <p className='text-[10px] text-muted-foreground flex items-center gap-1'>
                  ...
                  {t('tasks.taskLogs.newValue.label')}
                </p>
                <div
                  className={cn(
                ...
                >
                  {renderLogFieldValue(change.field, change.newValue, {
                    task,
                    t,
                  })}
                </div>
        ...
        {(!log.changes || log.changes.length === 0) && (
          <div className='text-[11px] text-muted-foreground text-center py-6 rounded-xl border border-dashed bg-muted/40'>
            {t('tasks.taskLogs.noChangesForLog')}
          </div>
        )}
      </div>
    </div>
